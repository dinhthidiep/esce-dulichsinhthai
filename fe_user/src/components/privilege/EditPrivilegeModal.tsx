...
const EditPrivilegeModal: React.FC<EditPrivilegeModalProps> = ({
  ...
  return (
    <div className="edit-privilege-modal-overlay" onClick={onClose}>
      <div className="edit-privilege-modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="edit-privilege-modal-header">
          ...
          <button className="edit-privilege-modal-close" onClick={onClose}>
        ...
        <div className="edit-privilege-modal-body">
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="edit-privilege-field">
              <label htmlFor="edit-privilege-serviceName">
                Tên dịch vụ
                <span className="edit-privilege-required-indicator">*</span>
              </label>
              <input
                id="edit-privilege-serviceName"
                name="serviceName"
                type="text"
                required
                ...
                autoComplete="off"
                className={localErrors.serviceName || errors.serviceName ? 'edit-privilege-input-error' : ''}
              />
              {(localErrors.serviceName || errors.serviceName) && (
                <div className="edit-privilege-error">{localErrors.serviceName || errors.serviceName}</div>
              )}
            </div>

            {/* Rank Field */}
            <div className="edit-privilege-field">
              <label htmlFor="edit-privilege-rank">
                Hạng người dùng
                <span className="edit-privilege-required-indicator">*</span>
              </label>
              <select
                id="edit-privilege-rank"
                name="rank"
                required
                value={localFormData.rank}
                onChange={handleInputChange}
                className={localErrors.rank || errors.rank ? 'edit-privilege-input-error' : ''}
              >
              ...
              {(localErrors.rank || errors.rank) && (
                <div className="edit-privilege-error">{localErrors.rank || errors.rank}</div>
              )}
            </div>

            {/* User Type Field */}
            <div className="edit-privilege-field">
              <label htmlFor="edit-privilege-userType">
                Loại người dùng
                <span className="edit-privilege-required-indicator">*</span>
              </label>
              <select
                id="edit-privilege-userType"
                name="userType"
                required
                value={localFormData.userType}
                onChange={handleInputChange}
                className={localErrors.userType || errors.userType ? 'edit-privilege-input-error' : ''}
              >
                <option value="">--Chọn--</option>
              ...
              {(localErrors.userType || errors.userType) && (
                <div className="edit-privilege-error">{localErrors.userType || errors.userType}</div>
              )}
            </div>
            ...
            <div className="edit-privilege-form-action">
              ...
              <button type="button" className="edit-privilege-btn-secondary" onClick={onClose} disabled={isSubmitting}>
                Đóng
              </button>
            </div>
          </form>
