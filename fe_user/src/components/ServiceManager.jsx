...
const ServiceManager = () => {
  ...
  useEffect(() => {
    const loadUserInfo = async () => {
      // Try to fetch fresh user info from database
      try {
        ...
        if (currentUser) {
          ...
          // If API call failed (e.g., 500 error), fall back to localStorage
        ...
        // If getCurrentUser throws, fall back to localStorage
        console.warn('Error fetching current user from database, using localStorage:', err);
        const storedUserInfo = localStorage.getItem('userInfo');
        if (storedUserInfo) {
          try {
            const user = JSON.parse(storedUserInfo);
            ...
            console.error('Error parsing user info from localStorage:', parseErr);
          }
        }
      }
    };
