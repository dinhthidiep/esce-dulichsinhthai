...
const ServicesPage = () => {
  ...
  // Fetch ratings for all services (cả Service và ServiceCombo)
  useEffect(() => {
    const fetchRatings = async () => {
      ...
      // Fetch ratings cho Service (nếu cần)
      // TODO: Thêm logic fetch ratings cho Service nếu backend có endpoint tương tự
    }
  ...
  const allServices = useMemo(() => {
    if (showServices) {
      ...
      // Filter các service có status = 'approved' hoặc 'open' (theo API response)
      const mappedServices: ServiceItem[] = tours
        .filter((tour: ServiceComboResponse) => {
          ...
          return status === 'approved' || status === 'open'
        })
