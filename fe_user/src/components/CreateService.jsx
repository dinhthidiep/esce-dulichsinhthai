...
const CreateService = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        setErrors(prev => ({ ...prev, image: 'Chỉ chấp nhận file ảnh (JPG, PNG, GIF, WebP)' }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serviceId = result?.service?.Id || result?.service?.id || result?.id || result?.Id;
      
      // Show success message regardless (if we got here, the request succeeded)
      alert('Dịch vụ được tạo thành công, vui lòng đợi xác nhận từ ban quản lý!');
        ...
        <div className="form-content">
          ...
          <form id="createServiceForm" onSubmit={handleSubmit} noValidate>
            ...
            <div className="field">
              ...
              {imagePreview && (
                <img
                  ...
                  alt="Xem trước ảnh"
                  loading="lazy"
                  style={{ maxWidth: '300px', maxHeight: '300px', marginTop: '10px', borderRadius: '4px' }}
                />
              )}
            </div>
            ...
            <div className="form-action">
              <button type="submit" className="primary" disabled={isLoading || isSubmitting || !formData.name || !formData.price}>
                {isLoading || isSubmitting ? 'Đang xử lý...' : 'Tạo dịch vụ'}
              </button>
              <button type="button" className="secondary" onClick={handleGoBack} disabled={isLoading || isSubmitting}>
                Quay lại
              </button>
            </div>
          </form>
