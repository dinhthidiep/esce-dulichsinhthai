...
const HostDashboard = () => {
    ...
    const [activeTab, setActiveTab] = useState(
        location.state?.activeTab || "services"
    );
    ...
    useEffect(() => {
        if (activeTab === "coupons" && couponManagementRef.current) {
            const currentToggleState =
                couponManagementRef.current.getToggleState();
            setCouponToggleState(currentToggleState);
        }
    }, [activeTab]);
    // Apply Promotion Modal states
    const [isApplyPromotionModalOpen, setIsApplyPromotionModalOpen] =
        useState(false);
    ...
    const [applyPromotionErrors, setApplyPromotionErrors] = useState<
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            try {
                ...
                console.log(
                    " HostDashboard: Nhận được dữ liệu user:",
                    response.data
                );
                ...
                if (
                    ...
                    setError(
                        "Bạn không có quyền xem thông tin này. Vui lòng đăng nhập lại."
                    );
                    navigate("/login", { state: { returnUrl: "/profile" } });
    ...
    const handleApplyPromotionSubmit = async (e) => {
        ...
        try {
            if (activeTab === "services") {
                ...
                console.log(
                    "Applying service promotion:",
                    applyPromotionFormData
                );
                ...
                try {
                    const serviceResponse = await axiosInstance.get(
                        `${API_ENDPOINTS.SERVICE}/${applyPromotionFormData.serviceId}`
                    );
                    ...
                    const newRule = {
                        ...
                        ServiceName: service
                            ...
                            : `Dịch vụ ID: ${applyPromotionFormData.serviceId}`,
                    };
                ...
                try {
                    const couponResponse = await axiosInstance.get(
                        `${API_ENDPOINTS.COUPON}/${applyPromotionFormData.couponId}`
                    );
                    ...
                    const newRule = {
                        ...
                        CouponCode: coupon
                            ...
                            : `Mã giảm giá ID: ${applyPromotionFormData.couponId}`,
                    };
            ...
            setError(
                (err as Error).message ||
                    "Có lỗi xảy ra khi áp dụng ưu đãi. Vui lòng thử lại."
            );
        ...
        try {
            ...
            if (Object.keys(newErrors).length > 0) {
                ...
                const firstErrorField = document.querySelector(
                    '.host-form-input[aria-invalid="true"]'
                );
                if (firstErrorField) {
            ...
            if (formData.dob) {
                ...
                if (!isNaN(dobDate.getTime())) {
                    ...
                    const month = String(dobDate.getMonth() + 1).padStart(
                        2,
                        "0"
                    );
            ...
            const currentPhone = getValue(
                formData.phone,
                userInfo?.Phone || userInfo?.phone
            );
            const currentGender = getValue(
                formData.gender,
                userInfo?.Gender || userInfo?.gender
            );
            const currentAddress = getValue(
                formData.address,
            ...
            console.log(
                " HostDashboard.handleSave: Payload sẽ gửi đến backend:",
                JSON.stringify(updateData, null, 2)
            );
            console.log(
                " HostDashboard.handleSave: Endpoint:",
            ...
            console.error(
                " HostDashboard.handleSave: Lỗi khi cập nhật thông tin:",
                err
            );
            ...
            if (err.response?.status === 401 || err.response?.status === 403) {
                const errorMsg =
                    "Bạn không có quyền cập nhật thông tin. Vui lòng đăng nhập lại.";
                setError(errorMsg);
                navigate("/login", { state: { returnUrl: "/profile" } });
        ...
        reader.onloadend = () => {
            setFormData(
                (prev: {
                    ...
                    const newDataString = JSON.stringify(newData);
                    setHasChanges(
                        newDataString !== originalFormDataRef.current
                    );
                    return newData as {
                        name: string;
                        email: string;
    ...
    const handleTabChange = (tab) => {
        ...
        if (isEditing && hasChanges) {
            const confirmChange = window.confirm(
                "Bạn có thay đổi chưa lưu. Bạn có chắc muốn chuyển tab?"
            );
            if (!confirmChange) {
                return;
            }
    ...
    const avatarUrl =
        formData.avatar || userInfo?.Avatar || userInfo?.avatar || "";
        ...
        <>
                ...
                <div className="host-profile-container">
                    ...
                    <div className="host-hostdashboard-content">
                        <div className="host-profile-content-header">
                            <h1 className="host-profile-title">
                                {getTabTitle()}
                            </h1>
                            {activeTab === "services" && (
                                <Button
                                    variant="default"
                                    onClick={() =>
                                        servicesManagementRef.current?.openCreateModal()
                                    }
                                ...
                                <>
                                    {couponToggleState ? (
                                        ...
                                        <Button
                                            variant="default"
                                            onClick={() =>
                                                couponManagementRef.current?.openCreateModal()
                                            }
                                            className="host-edit-button"
                                        >
                                            Tạo mới
                            ...
                            {activeTab === "service-combos" && (
                                <Button
                                    variant="default"
                                    onClick={() =>
                                        serviceComboManagementRef.current?.openCreateModal()
                                    }
                                    className="host-edit-button"
                                >
                                    Tạo combo mới
                            ...
                            {activeTab === "promotions" && (
                                <Button
                                    variant="default"
                                    onClick={() =>
                                        privilegeManagementRef.current?.openCreateModal()
                                    }
                                    className="host-edit-button"
                                >
                                    Tạo ưu đãi
                        ...
                        {error && (
                            <div
                                className="host-alert host-alert-error"
                                role="host-alert"
                            >
                        ...
                        {success && (
                            <div
                                className="host-alert host-alert-success"
                                role="host-alert"
                            >
                                    ...
                                    <div className="host-avatar-section-compact">
                                        <div className="host-avatar-wrapper-compact">
                                            ...
                                            <label
                                                ...
                                                className="host-avatar-change-button"
                                            >
                                                <svg
                                                    width="16"
                                                    height="16"
                                ...
                                <div className="host-profile-fields-grid">
                                    ...
                                    <div className="host-profile-fields-column">
                                        <div className="host-form-field-compact">
                                            <label
                                                htmlFor="name"
                                                className="host-form-label-compact"
                                            >
                                                <UserIcon className="host-field-icon" />
                                                Họ và tên{" "}
                                                <span className="host-required">
                                                    *
                                                </span>
                                            </label>
                                            <input
                        ...
                        {activeTab === "notifications" && (
                            <div className="host-tab-content">
                                <div className="host-empty-state">
                                    ...
                                    <p>
                                        Chức năng thông báo đang được phát
                            ...
                            <div className="host-tab-content">
                                <CouponManagement
                                    ...
                                    onApplyPromotionClick={() =>
                                        setIsApplyPromotionModalOpen(true)
                                    }
            ...
            {isApplyPromotionModalOpen && (
                <div
                    ...
                    onClick={handleCloseApplyPromotionModal}
                >
                    <div
                        className="host-modal-content host-create-service-modal"
                    ...
                    >
                        ...
                        <div className="host-modal-body">
                            <div className="host-disclaimer-text">
                                (
                                <span className="host-required-indicator">
                                    *
                                </span>
                                ) bắt buộc
                            </div>
                            ...
                            <form
                                onSubmit={handleApplyPromotionSubmit}
                                noValidate
                            >
                                ...
                                {activeTab === "services" ? (
                                    <div className="host-field">
                                        ...
                                        <select
                                            ...
                                            name="serviceId"
                                            value={
                                                applyPromotionFormData.serviceId
                                            }
                                            onChange={
                                                handleApplyPromotionInputChange
                                            }
                                        ...
                                        {applyPromotionErrors.serviceId && (
                                    ...
                                    <div className="host-field">
                                        ...
                                        <select
                                            ...
                                            name="couponId"
                                            value={
                                                applyPromotionFormData.couponId
                                            }
                                            onChange={
                                                handleApplyPromotionInputChange
                                            }
                                ...
                                <div className="host-form-action">
                                    <button
                                        ...
                                        disabled={
                                            isApplyingPromotion ||
                                            !applyPromotionFormData.serviceId ||
                                            !applyPromotionFormData.rankId ||
                                            !applyPromotionFormData.userType
                                        }
                                    >
                                        {isApplyingPromotion
                                            ? "Đang xử lý..."
