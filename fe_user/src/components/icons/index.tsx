...
export const StarIcon = ({ className = '', filled = false, half = false, ...props }: StarIconProps) => {
  // Tạo ID duy nhất cho mỗi sao nửa để tránh conflict
  const clipId = useRef(`star-half-${Math.random().toString(36).substring(2, 9)}`).current
...
export const HeartIcon = ({ className = '', filled = false, ...props }: IconProps & { filled?: boolean }) => (
  <svg
    className={className}
    width="20"
    height="20"
...
export const BookmarkIcon = ({ className = '', filled = false, ...props }: IconProps & { filled?: boolean }) => (
  <svg
    className={className}
    width="20"
    height="20"
