...
export const createService = async (formData) => {
  ...
  if (!hostId) {
    try {
      const { getCurrentUser } = await import('./SocialMediaApi');
  ...
  const body = {
    ...
    Description: formData instanceof FormData ? (formData.get('description') || null) : (formData.description || null),
    Price: formData instanceof FormData ? parseFloat(formData.get('price') || '0') : parseFloat(formData.price || '0'),
    HostId: hostId,
...
export const updateService = async (formData) => {
  ...
  let imageUrl = null;
  const imageFile = formData instanceof FormData ? formData.get('image') : formData.image;
  if (imageFile instanceof File) {
    const { uploadImageToFirebase } = await import('../services/firebaseStorage');
    imageUrl = await uploadImageToFirebase(imageFile, 'services');
  }
...
export const getServiceComboDetailById = async (detailId) => {
  ...
  if (!res.ok) throw new Error(await res.text() || 'Failed to load service combo detail');
  return res.json();
};
...
export const deleteServiceComboDetailsByComboId = async (comboId) => {
  ...
  if (!res.ok) throw new Error(await res.text() || 'Failed to delete service combo details');
  const text = await res.text();
  try { return JSON.parse(text); } catch { return { message: text || 'Service combo details deleted successfully' }; }
};
