  ...
  if (!response.ok) {
    if (response.status === 500) {
      try {
        ...
        console.warn('Backend returned 500 for /api/Review (likely object cycle):', errorText);
      } catch (e) {
        console.warn('Backend returned 500 for /api/Review and response body could not be read', e);
      }
      return [];
    }
