...
export const getAllBookings = async () => {
  ...
  if (!response.ok) {
    // Backend can throw a JsonException due to object cycles (User -> Role -> Accounts -> Role ...).
    // We are NOT allowed to change backend serialization, so treat 500 as "no bookings"
    // and let the UI keep working instead of crashing.
    if (response.status === 500) {
      try {
        const errorText = await response.text();
        console.warn('Backend returned 500 for /api/booking (likely object cycle):', errorText);
      } catch (e) {
        console.warn('Backend returned 500 for /api/booking and response body could not be read', e);
      }
      return [];
    }
