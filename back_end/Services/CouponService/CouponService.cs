...
namespace ESCE_SYSTEM.Services
{
    public class CouponService : ICouponService
    {
        ...
        // CreateAsync mới
        public async Task<Coupon> CreateAsync(CreateCouponDto couponDto)
        {
            var coupon = new Coupon
            {
                ...
                UsageCount = 0, // Luôn khởi tạo là 0
                HostId = couponDto.HostId,
                ServiceComboId = couponDto.ServiceComboId,
        ...
        // UpdateAsync mới
        public async Task<Coupon?> UpdateAsync(int id, UpdateCouponDto couponDto)
        {
            var existing = await _repository.GetByIdAsync(id);
            if (existing == null) return null;

            existing.Code = couponDto.Code ?? existing.Code;
            existing.Description = couponDto.Description ?? existing.Description;
            existing.DiscountPercent = couponDto.DiscountPercent ?? existing.DiscountPercent;
            existing.DiscountAmount = couponDto.DiscountAmount ?? existing.DiscountAmount;
            existing.UsageLimit = couponDto.UsageLimit ?? existing.UsageLimit;
            existing.ServiceComboId = couponDto.ServiceComboId ?? existing.ServiceComboId;
            existing.IsActive = couponDto.IsActive ?? existing.IsActive;
            existing.ExpiryDate = couponDto.ExpiryDate ?? existing.ExpiryDate;
