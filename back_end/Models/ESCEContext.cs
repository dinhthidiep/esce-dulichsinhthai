...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see http://go.microsoft.com/fwlink/?LinkId=723263.
                // Đã loại bỏ chuỗi kết nối ở đây, vì nó được cấu hình trong Program.cs
            }
        }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<Account>(entity =>
            {
                ...
                // --- Cột mới từ Account Model ---
                entity.Property(e => e.Level).HasColumnName("LEVEL").HasDefaultValueSql("((0))");
                entity.Property(e => e.TotalSpent).HasColumnType("decimal(18, 2)").HasColumnName("TotalSpent").HasDefaultValueSql("((0.00))");
            ...
            modelBuilder.Entity<Payment>(entity =>
            {
                ...
                // Cột BookingId (Nullable trong Model)
                ...
                // === ÁNH XẠ CHO CÁC CỘT MỚI (Đã Thêm) ===

                // 1. UserId (int?)
                entity.Property(e => e.UserId).HasColumnName("USER_ID").IsRequired(false);

                // 2. PaymentType (string?)
                entity.Property(e => e.PaymentType).HasColumnName("PAYMENT_TYPE");

                // 3. UpgradeType (string?)
                entity.Property(e => e.UpgradeType).HasColumnName("UPGRADE_TYPE");

                // 4. TransactionId (giữ nguyên ánh xạ)
                entity.Property(e => e.TransactionId).HasMaxLength(50).IsUnicode(false).HasColumnName("TRANSACTION_ID");

                // THIẾT LẬP FOREIGN KEY CHO BOOKING 
                entity.HasOne(d => d.Booking)
                .WithMany(p => p.Payments)
                .HasForeignKey(d => d.BookingId)
                .HasConstraintName("FK__PAYMENTS__BOOKIN__6477ECF3");

                // THIẾT LẬP FOREIGN KEY CHO USER (Đã Thêm)
                entity.HasOne(d => d.User)
                .WithMany(p => p.Payments)
                .HasForeignKey(d => d.UserId)
