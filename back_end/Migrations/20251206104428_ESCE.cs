...
namespace ESCE_SYSTEM.Migrations
{
    public partial class ESCE : Migration
    {
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            // Drop cột TransactionId nếu tồn tại
            migrationBuilder.Sql(@"
                IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('PAYMENTS') AND name = 'TransactionId')
                BEGIN
                    ALTER TABLE [PAYMENTS] DROP COLUMN [TransactionId];
                END
            ");

            // Drop cột UpdatedAt nếu tồn tại
            migrationBuilder.Sql(@"
                IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('PAYMENTS') AND name = 'UpdatedAt')
                BEGIN
