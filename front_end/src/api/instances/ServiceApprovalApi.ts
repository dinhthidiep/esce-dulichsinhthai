...
const normalizeServiceCombo = (payload: any): ServiceComboForApproval => {
  return {
    ...
    hostName: payload?.hostName ?? payload?.HostName ?? payload?.host?.name ?? payload?.host?.Name ?? payload?.Host?.name ?? payload?.Host?.Name ?? null,
    hostEmail: payload?.hostEmail ?? payload?.HostEmail ?? payload?.host?.email ?? payload?.host?.Email ?? payload?.Host?.email ?? payload?.Host?.Email ?? null,
