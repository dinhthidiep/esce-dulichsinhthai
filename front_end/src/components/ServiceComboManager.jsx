  ...
  const handleDeleteServiceCombo = async (serviceComboId) => {
    if (window.confirm('Bạn có chắc chắn muốn xóa combo dịch vụ này? Hành động này không thể hoàn tác.')) {
      try {
        ...
        const imageUrl = serviceCombo?.Image || serviceCombo?.image || null;
        ...
        let errorMessage = error.message || 'Có lỗi xảy ra khi xóa combo dịch vụ. Vui lòng thử lại.';
        
        // Check for foreign key constraint errors
        if (errorMessage.includes('foreign key') || errorMessage.includes('FOREIGN KEY') || 
            errorMessage.includes('constraint') || errorMessage.includes('CONSTRAINT') ||
            errorMessage.includes('reference') || errorMessage.includes('REFERENCE')) {
          errorMessage = 'Không thể xóa combo dịch vụ này vì đang có coupon hoặc booking liên quan. Vui lòng xóa các coupon và booking trước.';
        }
                ...
                filteredServices.map(s => (
                  <div key={s.Id || s.id} className="servicecombo-card">
                    <div className="service-image">
                      {(() => {
                        const name = s.Image || s.image || '';
                        const isAbsolute = name.startsWith('data:image') || name.startsWith('http://') || name.startsWith('https://');
                        const candidates = [];
                        if (name && name.trim() !== '') {
                          if (isAbsolute) {
                            ...
                            candidates.push(`${publicUrl}/img/uploads/${name}`);
                            // 2) Backend common static candidates (if served)
                            candidates.push(`${backend_url}/img/uploads/${name}`);
                            candidates.push(`${backend_url}/images/${name}`);
                          }
                        }
