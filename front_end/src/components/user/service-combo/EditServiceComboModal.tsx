...
import './EditServiceComboModal.css';

interface EditServiceComboModalProps {
  isOpen: boolean;
  onClose: () => void;
  loading: boolean;
  formData: {
  ...
  allServices: any[];
  selectedServices: Record<string, { selected: boolean; quantity: number; detailId?: string }>;
  servicesPage: number;
...
const EditServiceComboModal: React.FC<EditServiceComboModalProps> = ({
  isOpen,
  onClose,
  loading,
  formData,
  ...
  if (serviceFilterName.trim() !== '') {
    filteredServices = filteredServices.filter(s => 
      (s.Name || s.name || '').toLowerCase().includes(serviceFilterName.toLowerCase().trim())
    );
  }
          ...
          {loading ? (
            ...
            <>
              ...
              <form onSubmit={onSubmit} noValidate>
                ...
                {/* Image Upload Field - Multiple Images (max 10) */}
                <div className="combo-edit-edit-service-combo-field">
                  <label className="combo-edit-edit-service-combo-label">
                    Hình ảnh (tối đa 10 ảnh, mỗi ảnh tối đa 5MB)
                  </label>
                  
                  {/* Drag & Drop Area */}
                  <div
                    className={`combo-edit-upload-area ${isDragging ? 'combo-edit-dragging' : ''} ${errors.images ? 'combo-edit-error' : ''}`}
                  ...
                  >
                    ...
                    <div className="combo-edit-upload-content">
                      <ImageIcon className="combo-edit-upload-icon" />
                      <p className="combo-edit-upload-text">
                        Kéo thả ảnh vào đây hoặc <span className="combo-edit-upload-link">chọn từ máy tính</span>
                      </p>
                      <p className="combo-edit-upload-hint">
                  ...
                  {errors.images && (
                    <span className="combo-edit-form-error-text">{errors.images}</span>
                  )}
                  {imagePreviews.length > 0 && (
                    <div className="combo-edit-image-preview-grid">
                      {imagePreviews.map((preview, index) => (
                        <div key={index} className="combo-edit-image-preview-item">
                          <img
                            src={preview}
                            alt={`Preview ${index + 1}`}
                            className="combo-edit-image-preview"
                          />
                          {onRemoveImage && (
                            <button
                              type="button"
                              className="combo-edit-image-remove-btn"
                              onClick={(e) => {
                                e.stopPropagation();
                                onRemoveImage(index);
                              }}
...
export default EditServiceComboModal;
